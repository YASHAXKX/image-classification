    ...
    try:
        ...
        for (x, y, w, h) in faces:
            ...
            if isinstance(result, list) and len(result) > 0 and isinstance(result[0], pd.DataFrame) and not result[0].empty:
                ...
                confidence = 1 - result[0].iloc[0]['VGG-Face_cosine']  # Convert distance to confidence
                recognized_celebrities.append((celebrity_name, (x, y, w, h), confidence))
            else:
                recognized_celebrities.append(("Unknown", (x, y, w, h), 0))
        ...
        return [("An error occurred", (0, 0, 0, 0), 0)]
    ...
    try:
        ...
        for (x, y, w, h) in faces:
            ...
            if isinstance(result, list) and len(result) > 0 and isinstance(result[0], pd.DataFrame) and not result[0].empty:
                celebrity_name = extract_celebrity_name(result[0])
                confidence = 1 - result[0].iloc[0]['VGG-Face_cosine']  # Convert distance to confidence
                print(f"Match found: {celebrity_name} with confidence: {confidence:.2f}")
                recognized_celebrities.append((celebrity_name, (x, y, w, h), confidence))
            else:
    ...
    if st.session_state.page == 'home':
        ...
        if uploaded_file is not None:
            ...
            if st.button("Recognize Celebrities", key="recognize_button", use_container_width=True):
                with st.spinner("Recognizing..."):
                    ...
                    for celebrity, (x, y, w, h), confidence in recognized_celebrities:
                        cv2.rectangle(img_with_boxes, (x, y), (x+w, y+h), (0, 255, 0), 2)
                        cv2.putText(img_with_boxes, f"{celebrity} ({confidence:.2f})", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                    
                    with col2:
                        st.image(img_with_boxes, caption="Recognized Celebrities", use_column_width=True)
                    
                    st.markdown(f'<p class="prediction-label">Recognized Celebrities: {", ".join([f"{celeb} ({conf:.2f})" for celeb, _, conf in recognized_celebrities])}</p>', unsafe_allow_html=True)
        ...
        try:
            ...
            while run:
                ...
                for celebrity, (x, y, w, h), confidence in current_celebrities:
                    cv2.rectangle(frame_with_labels, (x, y), (x+w, y+h), (0, 255, 0), 2)
                    cv2.putText(frame_with_labels, f"{celebrity} ({confidence:.2f})", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
